...
public class LITPlibvirtTestRunner extends TorTestCaseHelper {
    
    Logger logger = Logger.getLogger(LITPlibvirtTestRunner.class);
    ...
    @TestId(id = "CXP9031115-1", title = "Upgrade ERIClitplibvirt rpm on the MS")
    @Test(groups={"ACCEPTANCE"})
    public void upgradeERIClitplibvirtRPM() throws TimeoutException, FileNotFoundException {

        rpmUpgradeOperator.initialise("libvirt");
     ...
     * @PRIORITY HIGH
     */
    @TestId(id = "CXP9031115-2", title = "Run python test cases for ERIClitplibvirt package")
    @Test(groups={"CDB_REGRESSION", "ACCEPTANCE"})
    public void runERIClitplibvirtTests() {
     ...
     * @PRE Execution of test {@link #runERIClitplibvirtTests()}
     * @PRIORITY HIGH
     * @param className
     * @param name
     * @param failures
     */
    @TestId(id = "CXP9031115-3", title = "Parse xml outputs from python tests")
